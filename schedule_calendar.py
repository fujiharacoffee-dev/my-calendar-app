...
with st.sidebar.expander("📅 予定・猫足の追加", expanded=True):
    ...
    # UIの切り替え：猫足にチェックが入るとテキスト入力欄を隠す
    if is_open:
        st.info("猫足の設定（この時テキスト入力はできません）")
        indiv_s = st.slider("猫足のサイズ", 30, 250, 100)
        indiv_x = st.slider("猫足の横位置", 0, 100, 50)
        indiv_y = st.slider("猫足の縦位置", 0, 100, 50)
    else:
        st.info("予定テキストの設定")
        t_font_size = st.slider("文字の大きさ", 10, 60, 22)
...
# 削除機能
if not df.empty:
    with st.sidebar.expander("🗑 登録済みの予定/猫足の削除"):
