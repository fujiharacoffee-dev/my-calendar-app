...
def load_data():
    if os.path.exists(DATA_FILE):
        try:
            ...
            cols = ["id", "日付", "時間", "タイトル", "スタンプ", "サイズ", "位置X", "位置Y", "文字サイズ", "色設定"]
            for col in cols:
                if col not in df.columns:
                    if col == "サイズ": df[col] = 100
                    elif col == "文字サイズ": df[col] = 22
                    elif col == "色設定": df[col] = "標準（黒・グレー）"
        ...
        for c, d in enumerate(week):
            ...
            if is_preview_day and preview_data["スタンプ"]:
                ...
                if not stamp_rows.empty:
                    ...
                    s_size, pos_x, pos_y = row.get('サイズ', 100), row.get('位置X', 50), row.get('位置Y', 50)
            ...
            if not day_items.empty:
                ...
                for _, row in day_items.iterrows():
                    ...
                    color_key = row.get('色設定', "標準（黒・グレー）")
                    colors = COLOR_MAP.get(color_key, COLOR_MAP["標準（黒・グレー）"])
                    
                    if t_val and t_val not in ["nan", ""]:
                        draw.text((x + 20, curr_ty), t_val, font=get_font(int(t_size * 0.8)), fill=colors["time"])
                        curr_ty += int(t_size * 0.8) + 5
                    
                    if title_val and title_val not in ["nan", ""]:
                        draw.text((x + 20, curr_ty), title_val[:12], font=get_font(int(t_size)), fill=colors["title"])
...
with st.sidebar.expander("📅 予定・猫足の追加", expanded=True):
    ...
    if is_open:
        ...
        selected_color_key = st.selectbox("文字の色", list(COLOR_MAP.keys()))
        
        time_mode = st.radio("時間入力形式", ["プルダウン", "自由入力", "なし"], horizontal=True)
        if time_mode == "プルダウン":
    ...
    if st.button("保存して更新"):
        new_row = pd.DataFrame([{
            ...
            "文字サイズ": t_font_size, "色設定": selected_color_key
        }])
